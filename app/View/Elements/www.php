                ...
                <div style="margin-bottom: 10px;">
                    <ul class="nav nav-tabs pattern">
                        ...
                        <li class="<?php echo (!empty($this->params['action']) && ($this->params['action'] == 'index') ) ? 'active' : 'inactive' ?>">
                            <?php
                            echo $this->Html->link(
                                    '<span><i class="icomoon-icon-home-8"></i>', array(
                                'action' => 'index'
                                    ), array(
                                'escape' => false
                                    )
                        ...
                        <?php foreach ($filteredData as $menu): 
                            //pr($menu);
                        //$m = $menu['menus'];
                            ?>
                            ...
                            <li class="<?php 
                             if (!empty($menu['sub_menu'])) {
                                    ?>dropdown <?php }?> <?php echo (!empty($this->params['action']) && ($this->params['action'] == 'livetv') ) ? 'active' : 'inactive' ?>">

                                <?php echo $this->Html->link($menu['menu'], array('action' => $menu['menu'])) ?>
                            ...
                            <?php 
                             if (!empty($menu['sub_menu'])) {
                                    ...
                                    <?php
                                    foreach ($menu['sub_menu'] as $submemu) {
                                        ?>
                                            <li class="<?php echo (!empty($this->params['action']) && ($this->params['action'] == $submemu) ) ? 'active' : 'inactive' ?>">
                                                <?php
                                                echo $this->Html->link($submemu, array('action' => $submemu));
                                                ?>
                                            </li>
                                            <?php
